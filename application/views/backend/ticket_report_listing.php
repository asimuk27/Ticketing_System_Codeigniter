...
<style>
  ...
  <div id="page-wrapper">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-12">
                        <div class="right_first_box"><img src="<?php echo $this->config->item('admin_image_path');?>home.png"><a href="#">Home</a> > <a href="#">Manage Ticket Reports</a></div>
                    </div>
                </div>
                             ...
                             <form class="form-horizontal" role="form" method="post" name="user_list_form" id="user_list_form">
                            	<div class="box-body">
									...
									<div class="form-group">
										...
										<div class="col-md-3">
											<input type="text" name="from_date" id="from_date" class="space_right form-control" <?php if(isset($_POST) && !empty($_POST['from_date'])){
											echo 'value="'.$_POST['from_date'].'"';}else{ echo ""; } ?> readonly>
									    </div>

									    <label for="to_date" class="col-sm-2 control-label">To Date</label>
										<div class="col-md-3">
											 <input type="text" name="to_date" id="to_date" class="space_right form-control" <?php if(isset($_POST) && !empty($_POST['to_date'])){
									...
									<div class="form-group">
									 ...
									 <label for="user_email" class="col-sm-2 control-label">User Email</label>
										<div class="col-md-3">
											<input type="text" name="user_email" id="user_email" class="space_right form-control" <?php if(isset($_POST) && !empty($_POST['user_email'])){
											echo 'value="'.$_POST['user_email'].'"';}else{ echo ""; } ?>>
										</div>
									</div>
								</div>
								...
								<div class="form-group">
									<div class="col-sm-offset-2 col-md-10">
										...
										<a  class="btn special_class" href="<?php echo $this->config->item("base_url");?>backend/ticket_reports/refresh_listing">Reset</a>
									</div>
								</div>
							 </form>
                                </div>
                ...
                <div class="row margin_top">
                   ...
                   <div class="blue_top">Ticket Report Listing</div>
						...
						foreach($data as $user_data){ ?>
							<tr>
                                <td>
                                    <?php
                                        if( $user_data['price'] ) {
                                            echo ltrim($user_data['order_id'], '0');
                                        } else {
                                            echo '';
                                        }
                                    ?>
								...
								<td><?php echo $user_data['charity_name'];?></td>
								<td><?php echo $user_data['title'];?></td>
								<td><?php echo $user_data['schedule_title'];?></td>
                                <td><?php echo $user_data['quantity'];?></td>
                                <?php if( $user_data['price'] ) { ?>
							        <td><?php echo number_format($user_data['price'] * $user_data['quantity'],2);?></td>
                                <?php } else { ?>
                                    <td>0.0</td>
                                <?php } ?>
                                <td><?php echo $user_data['ticket_name'];?></td>
                                <?php if( $user_data['price'] ) { ?>
                                    <td>
                                        <a href="<?php echo base_url(); ?>assets/tickets_generated/<?php echo $user_data['order_id'];?>.pdf">View</a>
                                    </td>
                                <?php } else { ?>
                                    <td></td>
                                <?php } ?>
...
<script>
	function reset_page(){
		window.location.href = '<?php echo base_url(); ?>index.php/backend/ticket_reports';
	}
...
$(document).ready(function(){
      ...
      $('form').attr('action','<?php echo base_url(); ?>index.php/backend/ticket_reports/export_ticket_csv');
      $('form').submit();
    });
